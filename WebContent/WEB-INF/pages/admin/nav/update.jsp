		...
		<c:if test="${!empty msg}">
		   <div class="alert alert-error" style="font-size:2;">${msg }</div>
			...
			<div class="succ-toggle">
				<form id="_article-form" action="${pageContext.request.contextPath }/admin/nav/update" method="post" >
					<input type="hidden" name="id" value=${c.id } readonly="readonly"/>
					<div class="control-group">
					<label class="control-label" for="title">名称</label>
					<input type="text" name="name" id="name"  placeholder="导航名称"  value="${c.name }"  class="input-block-level">
					...
					<textarea name="description" placeholder="描述" rows="2" class="input-block-level">${c.description }</textarea>
					<span id="msg-description" class="help-block"></span>
					</div>
					
					<div class="control-group">
					<label class="control-label" for="title">排序</label>
					<input type="text" name="rank" id="rank" value="${c.rank }" placeholder="排序" value="0" class="input-block-level">
					<span id="msg-title" class="help-block"></span>
					</div>
					
					<div class="control-group">
					<button type="submit" class="btn btn-primary pull-right" id="submit">保存</button>
					</div>
				</form>
